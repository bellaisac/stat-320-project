...
::: {.smaller}
- There is a skew and outliers within the small sample sized groups ($n_{casual}$ = 24, $n_{elite}$ = 19) making a median the most accurate measure of center
- T-Distribution and randomized equivalents would not be appropriate as a model of the null distribution with these conditions. 
- We are planning on running a Wilcoxon Rank Sum Test (WRST) to obtain our test statistics and CI.  
:::
...
## Summary Statistics
```{r}
df_stats(Actual_Mean_Go_RT_ms~Game, data = nerds) |> 
  select(Game, mean, median, sd, n) |> 
  knitr::kable()
```
